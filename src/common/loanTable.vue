<template>
	<div class="shift">
		...
		<div class="box"  v-else v-for="(item, index) in oneOrder" :key='index' :class="{'states1': item.status === '0', 'states2': item.status ==='1', 'states3': item.status ==='2', 'states4': item.status ==='100','states5': item.status ==='4' }" @click='(Number(item.status) ===100 ? false: true) && showDetailInfo(item.sub_bill_id, item.status)'>
			<img v-show='Number(item.type) === 1' :src="imgIcon[0]" alt="" class="icon">
			<img  v-show='Number(item.type) === 2' :src="imgIcon[1]" alt="" class="icon">
			<div class="top">
				<div class="state">
					<img :src="imgSrc[4]" width="32" height="32" alt="" v-if="item.st ==='已冻结'">
					<img :src="imgSrc[1]" width="32" height="32" alt="" v-else-if="item.st ==='等待上传凭证'">
					<img :src="imgSrc[0]" width="32" height="32" alt="" v-else-if="item.st ==='等待确认'">
					<img :src="imgSrc[2]" width="32" height="32" alt="" v-else-if="item.st ==='已完成'">
					<img :src="imgSrc[3]" width="32" height="32" alt="" v-else="item.st ==='管理员审核中'">
				</div>
				<div class="table">
					<p class="item1">号码：{{item.idName}}</p>
					<p class="item2">状态：{{item.st}} <span class='dTimeBox' >{{item.dTime}}</span></p>
					<p class="item3">
				...
				<!-- <div class="env"><img :src="env" width="27" height="21" alt=""></div> -->
			</div>
		...
		<div class="moreData" v-show="oneOrder.length>0" style="text-align:center;color:#ac9375;font-size:12px;padding: 15px 0">
...
<script>
import { loanOneAccounts } from '@/api/index'
	...
	export default {
		...
		methods: {
			...
			showDetailInfo(id, status) {
				// console.log(id);
				this.$router.push('/home/page/lfdinfo?id='+  id +'&st=' + status);
			},
			getFdinfo() {
				this.showLoading();
						...
						if(res.data!==null){
							for(let i=0;i< res.data.length;i++){
								...
								if(res.data[i].status=='0'){
									...
									info.idName = (info.type == 1 ? 'B' : 'S')+moment(res.data[i].time).format("HHmmss") + md5(res.data[i].sub_bill_id).substr(8,4);
									info.dTime = '';
									...
									if(!!this.oneOrder && Number(info.status)===1)
										this.dealTime(this.oneOrder[this.oneOrder.length-1],res.data[i].time, res.data[i].current_time, res.data[i].interval);
							}
						}else{
							this.oneOrder = [];
						}
